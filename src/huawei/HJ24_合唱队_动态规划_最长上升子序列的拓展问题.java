...
public class HJ24_合唱队_动态规划_最长上升子序列的拓展问题 {
    ...
    //
    // f[n] 表示以第 n 个字符结尾的最长上升子序列，那么最终结果为
    private static int[] maxSubUpperLen(int[] nums) {

        // records[i] 表示以第 i 个字符为终点的最大上升子序列
    ...
    // 求 cs 以第 n 个字符结尾的最长上升子序列，并记录到 record 中
    // f[n] = f[n-1] + 1 ; if cs[n] > cs[n-1]
    // f[n] = 1  ; if cs[n] < cs[n-1]
    private static int upperFn(int[] cs, int[] f, int n) {

        int result = 1;
        for (int i = 0; i < n; i++) {
            int r_i = cs[n] > cs[i]? f[i] + 1 : 1; // 注意如果这里 cs[n] <= cs[i]，r_i 为 1，因为条件限定了必须带上最后一个字符的最长上升子序列
